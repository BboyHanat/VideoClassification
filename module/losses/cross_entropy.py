...
class CrossEntropyLoss(BaseWeightedLoss):
    ...
    and ``label``.
    1) Hard label: This label is an integer array and all the elements are
        in the range [0, num_classes - 1]. This label's shape should be
        ``cls_score``'s shape with the `num_classes` dimension removed.
    2) Soft label(probability distribution over classes): This label is a
        probability distribution and all the elements are in the range
    ...
    def _forward(self, cls_score, label, **kwargs):
        """
        Forward function.
        :param cls_score: The class score.
        :param label: The ground truth label.
        :param kwargs: Any keyword argument to be used to calculate CrossEntropy loss.
        :return: torch.Tensor: The returned CrossEntropy loss.
        """
        if cls_score.size() == label.size():
            ...
            if self.class_weight is not None:
                ...
                lsm = lsm * self.class_weight.unsqueeze(0)  # noqa
...
class BCELossWithLogits(BaseWeightedLoss):
    ...
    def __init__(self, loss_weight=1.0, class_weight=None):
        ...
        self.class_weight = None
        if class_weight is not None:
            self.class_weight = torch.Tensor(class_weight)  # noqa
    ...
    def _forward(self, cls_score, label, **kwargs):
        ...
        :param cls_score: The class score.
        :param label: The ground truth label.
        :param kwargs: Any keyword argument to be used to calculate bce loss with logits.
        :return: The returned bce loss with logits.
        """
        if self.class_weight is not None:
            assert 'weight' not in kwargs, "The key 'weight' already exists."
            kwargs['weight'] = self.class_weight.to(cls_score.device)
...
class CBFocalLoss(BaseWeightedLoss):
    ...
    """Class Balanced Focal Loss. Adapted from
    https://github.com/abhinanda-punnakkal/BABEL/.
     This loss is used in the skeleton-based action
    ...
    def _forward(self, cls_score, label, **kwargs):
        ...
        :param label: The ground truth label.
        :param kwargs: Any keyword argument to be used to calculate bce loss with logits.
        :return: torch.Tensor: The returned bce loss with logits.
        """
        ...
        if self.gamma:
            modulator = torch.exp(-self.gamma * label_one_hot * cls_score -
                                  self.gamma * torch.log(1 + torch.exp(-1.0 * cls_score))) # noqa
